...
static int IDT_8T49N24x_Configure_FreeRun(u32 I2CBaseAddress, u8 I2CSlaveAddress);
...
static int IDT_8T49N24x_CheckDeviceID(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	...
	/* Check. Accept both 0x0606 and 0x0607: the 8T49N24x family reports
	 * either depending on variant (this driver hard-coded 0x0606 while its
	 * own header defines DEVID as 0x0607). Accepting both avoids falsely
	 * rejecting a present chip. */
	if (DeviceId == 0x0606 || DeviceId == 0x0607)
		return XST_SUCCESS;
...
int IDT_8T49N24x_Init(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	...
	if (Result == XST_SUCCESS) {
		...
		/* Configure device for FREE-RUN synthesizer mode (no input reference).
		 * The AUBoard 8T49N241 that feeds the GT TX refclk has no incoming
		 * reference clock, so it must synthesize the output from its own 40 MHz
		 * crystal. Config_JA (jitter-attenuator) needs an input clock to lock
		 * its DPLL; with none present the DPLL never locks and the Q2/Q3 outputs
		 * are squelched -> GT sees no refclk (txRefHz=0). Config_Syn free-runs. */
		Result |= IDT_8T49N24x_Configure_FreeRun(I2CBaseAddress, I2CSlaveAddress);
...
/* Free-run synthesizer config: produces a 148.5 MHz clock on Q2/Q3 from the
 * internal 40 MHz crystal, with no input reference required. Mirrors
 * Configure_JA but writes the Config_Syn blob. */
static int IDT_8T49N24x_Configure_FreeRun(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	int Result = XST_SUCCESS;
	u32 Index;
	...
	for (Index=8; Index<sizeof(IDT_8T49N24x_Config_Syn); Index++) {
		/* Skip address 0x70 (enables the DPLL and APLL calibration; the caller
		 * toggles it around this write via IDT_8T49N24x_Enable). */
		if (Index != 0x070) {
			Result |= IDT_8T49N24x_SetRegister(I2CBaseAddress, I2CSlaveAddress,
...
int IDT_8T49N24x_EnableQ3ScopeProbe(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	/* Register 0x003D = OUTMODE3[2:0]<<5 | SE_MODE3<<4 | OUTMODE2[2:0]<<1 | SE_MODE2.
	 * Config_Syn leaves this 0x44 (OUTMODE3=010=LVDS, OUTMODE2=010=LVDS).
	 * Set OUTMODE3=011 (LVCMOS) so the unconnected Q3/nQ3 (pins 26/25) swings
	 * rail-to-rail with no termination, leaving OUTMODE2 (Q2) as LVDS:
	 *   0x003D = 0b0110_0100 = 0x64.
	 * Q3 is already enabled in OUTEN (0x0039) and its divider is set for the
	 * same frequency as Q2 by the active config. */
	return IDT_8T49N24x_SetRegister(I2CBaseAddress, I2CSlaveAddress, 0x003D, 0x64);
}

int IDT_8T49N24x_SetQ3DivideDown(u32 I2CBaseAddress, u8 I2CSlaveAddress,
                                 u32 Factor, u32 *Nq3Out)
{
	int Result;
	/* Read current Q2 integer divider N_Q2[17:0] (regs 0x45..0x47). */
	u32 nq2 = (((u32)IDT_8T49N24x_GetRegister(I2CBaseAddress, I2CSlaveAddress,
							0x0045) & 0x03) << 16)
			| ((u32)IDT_8T49N24x_GetRegister(I2CBaseAddress, I2CSlaveAddress,
							0x0046) << 8)
			|  (u32)IDT_8T49N24x_GetRegister(I2CBaseAddress, I2CSlaveAddress,
							0x0047);
	u32 nq3 = nq2 * Factor;
	if (nq3 > 0x3FFFF)            /* clamp to the 18-bit N_Qm field */
		nq3 = 0x3FFFF;

	/* Write N_Q3 (regs 0x48..0x4A); NFRAC_Q3 is left at the config's 0. */
	Result  = IDT_8T49N24x_SetRegister(I2CBaseAddress, I2CSlaveAddress,
							0x0048, (nq3 >> 16) & 0x03);
	Result |= IDT_8T49N24x_SetRegister(I2CBaseAddress, I2CSlaveAddress,
							0x0049, (nq3 >> 8) & 0xff);
