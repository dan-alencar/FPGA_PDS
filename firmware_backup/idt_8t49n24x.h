...
/* Diagnostic: switch the (unconnected) Q3/nQ3 output driver to LVCMOS so it
 * swings rail-to-rail with no external termination, for scope verification
 * that the PLL is actually generating a clock. Leaves Q2 untouched. */
int IDT_8T49N24x_EnableQ3ScopeProbe(u32 I2CBaseAddress, u8 I2CSlaveAddress);

/* Diagnostic: divide Q3 down by 'Factor' relative to Q2 by reading the current
 * Q2 integer divider (N_Q2) and writing N_Q3 = N_Q2 * Factor (NFRAC_Q3 stays 0
 * from the base config). This keeps the known-good VCO lock untouched and just
 * scales Q3 to a low, scope-visible frequency: Q3 = Q2_freq / Factor.
 * Returns the N_Q3 actually written (clamped to 18-bit) via *Nq3Out. */
