/*
 * EDID advertised by the HDMI RX subsystem to the upstream source.
 *
 * Xilinx generic HDMI 2.0 EDID (base block + CEA-861 extension), extracted
 * verbatim from the AUBoard HDMI Pass-Thru reference (xhdmi_edid.h). Advertises
 * common CEA modes up to 4k. Loaded at init via XV_HdmiRxSs_SetEdidParam().
 *
 * Referenced from main_passthrough.c as `extern const u8 Edid[256]`.
 * Replace with a sink-cloned EDID later if you want exact source-mode matching
 * (the reference does this via CloneTxEdid(): read TX sink EDID, LoadEdid to RX).
 */
#include "xil_types.h"
