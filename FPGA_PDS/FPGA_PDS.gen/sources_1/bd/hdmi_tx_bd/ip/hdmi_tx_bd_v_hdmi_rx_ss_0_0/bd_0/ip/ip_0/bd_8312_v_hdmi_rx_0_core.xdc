...
set_max_delay 15 -from [get_pins -of [get_cells -hier -filter {NAME=~ *aclk_dpram_reg*}] -filter {REF_PIN_NAME == CLK}] \
                   -to [get_pins -of [get_cells -hier -filter {NAME=~ *bclk_dout_reg*}]  -filter {REF_PIN_NAME == D}] \
                   -datapath_only
###
## Groups
###
...
create_waiver -scope -internal -user "hdmi" -tags "10914" -type CDC -id {CDC-11} -description "Can be waived in reset" \
...
create_waiver  -type CDC -id CDC-1 -user hdmi -from [get_pins -of [get_cells -hier -filter {NAME=~*aclk_dpram_reg*}] -filter {REF_PIN_NAME==CLK}] -to [get_pins -of [get_cells -hier -filter {NAME=~*bclk_dout_reg*}] -filter {REF_PIN_NAME==D}]  -description {Waiving CDC-1 warning because it is treating the asynchronous LUTRAM read to the read register path as CDC path which is not a real CDC} 
